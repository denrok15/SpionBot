    ...
    async def is_allowed(self, user_id: int) -> bool:
        ...
        if user_id in self._requests:
            while (
                ...
                and current_time - self._requests[user_id][0] > self.period
            ):
                self._requests[user_id].popleft()

        if (
            user_id in self._requests
            and len(self._requests[user_id]) >= self.max_requests
        ):
    ...
    def room_lock(self):
        ...
        def decorator(func):
            ...
            async def wrapper(
                ...
                logger.debug(
                    f"🔒 User {user_id} блокирует комнату {room_id} для {func.__name__}"
                )
                async with lock:
                    try:
                        result = await func(update, context, *args, **kwargs)
                        logger.debug(
                            f"✅ User {user_id} завершил {func.__name__} в комнате {room_id}"
                        )
                        return result
                    except Exception as e:
                        logger.error(
                            f"❌ Ошибка в {func.__name__} у user {user_id}: {e}"
                        )
                        raise
                ...
                if not await limiter.is_allowed(user_id):
                    ...
                    if time.time() - last_warning_time > 5:
                        try:
                            ...
                            logger.error(
                                f"Ошибка отправки rate limit предупреждения: {e}"
                            )
